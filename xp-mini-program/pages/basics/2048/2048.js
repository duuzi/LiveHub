...
Page({
  ...
  getRank:function(){
    util.request(api.GetGameRank).then(res => {
      this.setData({
        scoreList: res.data,
      })
    })
  ...
  move: function (e) {
    ...
    if (flag == 1) {
      let params = {score:this.data.score}
      util.request(api.UploadScore, params, 'POST').then(res => {
      })
      wx.showModal({
        title: '友好鼓励',
        content: '你是个天才吗？',
  ...
  onShareAppMessage: function () {
    var msg = '我在LiveHub2048,获得了' + this.data.score + '分!赶紧来试试吧'
    return {
      title: msg,
      path: '/page/home/home'
    }
